## ===== R: Remove seasonal/calendar components for the panel + safe CSV export =====
## Paths
...
## 1) Read the input (encoding-safe) - also checks the price variables
...
## 3) Settings (seasonality removed via week dummies, so no Fourier terms needed)
# include_month, K52, K13 settings removed (week dummies capture the seasonality)

## 4) Remove seasonal/calendar components by item (week dummies; multiplicative/additive) - price added
remove_season_calendar <- function(g){
  ...
  n <- nrow(g)
  # Week factor (52-week annual cycle) and holiday dummies
  ...
  # ---- multiplicative adjustment for quant (log model)
  ...
  # ---- multiplicative adjustment for price (log model) - added
  ...
  # ---- additive adjustment for price - added
...
## 5) Apply to all items & save (numeric conversion, NA -> blank) - price variables added
