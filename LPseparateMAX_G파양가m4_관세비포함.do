...
program define _lp_common_prep
    ...
    * 로그 변환
    replace i_price = ln(i_price_without_tariff)
    ...
    * 이벤트 시 강도(처리그룹만)
    ...
    sort qcode rtime
    rangestat (mean) intensity_temp, interval(rtime -12 0) by(qcode)
    ...
    * 좌변 Δ^h y := y_{t+h} - y_{t-1}
    if `h'==0 {
        ...
        * 필요하면 관측 수만 기록하고 계수는 0 또는 missing으로 두기
        ...
        local k = -`h'
        gen double `dY' = L`k'.i_price - L.i_price
...
twoway ///
    ...
    xlabel(-`Hpre'(2)`Hpost') xscale(range(-`Hpre' `Hpost')) ///
    xtitle("h (상대시점, 월)") ///
    ytitle("Log 수입가격 반응 (강도 1%p당)") ///
    legend(order(3 "처치그룹1" 4 "처치그룹2") pos(6) ring(0)) ///
    yline(0) xline(-1)
graph export LPseparateMAX파양가_Gm4-관세비포함.png, replace width(3000)
...
twoway ///
    ...
    (line  LP2 h,   lpattern(shortdash) lwidth(medthick) lcolor(maroon)), ///
    xlabel(-`Hpre'(2)`Hpost') xscale(range(-`Hpre' `Hpost')) ///
    xtitle("h (Relative Months)") ///
    ytitle("Log Import Price Response (per 1%p Intensity)") ///
    legend(order(3 "Treated Group 1" 4 "Treated Group 2") pos(6) ring(0)) ///
    yline(0) xline(-1)
graph export LPseparateMAX파양가_Gm4-관세비포함_eng.png, replace width(3000)
