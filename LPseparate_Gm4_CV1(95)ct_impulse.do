...
************************************************
** LP-DiD: separate estimation for the two groups, with intensity
************************************************
clear all
set more off
...
program define _lp_common_prep
    ...
    * Preserve the raw treatment path (for the D-bar dose calculation — before bridging)
    gen byte TRQD_raw = TRQD
...
postfile `post1' int h long N_all1 N_T1 N_ctrl1 ///
    double LP1 lb1 ub1 Dbar_raw1 Dbar_br1 using "`res1'", replace
...
forvalues h = -`Hpre'/`Hpost' {
    ...
    * Cumulative treated days over [t, t+h] (dose D-bar): computed on the full panel before any keep
    if `h' >= 0 {
        quietly rangestat (sum) TRQD_raw TRQD, interval(date 0 `h') by(qcode)
        rename TRQD_raw_sum `dsr'
        rename TRQD_sum `dsb'
    }
    else {
        gen double `dsr' = .
    ...
    * Average cumulative treated days across treated events (raw/bridged)
    local Draw = .
    local Dbr  = .
    if (`NT' > 0 & `h' >= 0) {
        quietly summarize `dsr' if `ev'==1 & d==1 & prevtr120==0
        local Draw = r(mean)
        quietly summarize `dsb' if `ev'==1 & d==1 & prevtr120==0
        local Dbr = r(mean)
    }
    ...
    if (`Nall'==0 | `NT'==0 | `Nc'==0) {
        ...
        gen double shock = intensity*`ev'*(d==1)*(prevtr120==0)
        ...
        * Wild cluster bootstrap CI only when usewb=1 (unstable in the tails)
        ...
        post `post1' (`h') (`Nall') (`NT') (`Nc') (`bb') (`lbv') (`ubv') (`Draw') (`Dbr')
    }
}
postclose `post1'
...
postfile `post2' int h long N_all2 N_T2 N_ctrl2 ///
    double LP2 lb2 ub2 Dbar_raw2 Dbar_br2 using "`res2'", replace
    ...
    * Average cumulative treated days across treated events (raw/bridged)
    ...
    if (`Nall'==0 | `NT'==0 | `Nc'==0) {
        ...
        * Fruit regressions: drop the climate interactions (4 current + 4 L365), keep oil x item
        ...
        post `post2' (`h') (`Nall') (`NT') (`Nc') (`bb') (`lbv') (`ubv') (`Draw') (`Dbr')
    }
}
postclose `post2'
...
export delimited using "LPseparate_Gm4_CV1(95)ct_impulse_res.csv", replace
...
twoway ///
    ...
    yline(0) xline(-1)
graph export "LPseparate_Gm4_CV1(95)ct_impulse.png", replace width(3000)
...
twoway ///
    ...
    yline(0) xline(-1)
graph export "LPseparate_Gm4_CV1(95)ct_impulse_eng.png", replace width(3000)

* Third image — English version with a short ytitle (in case the long ytitle gets clipped)
twoway ///
    (rarea ub1 lb1 h, color(navy%25)   lcolor(navy%60)) ///
    (rarea ub2 lb2 h, color(maroon%25) lcolor(maroon%60)) ///
    (line  LP1 h,   lwidth(medthick)    lcolor(navy)) ///
    ...
    xlabel(-`Hpre'(50)`Hpost') xscale(range(-`Hpre' `Hpost')) ///
    xtitle("h (Relative Days)") ///
    ytitle("Log Retail Price Response") ///
    legend(order(3 "Treated Group 1" 4 "Treated Group 2") pos(6) ring(0)) ///
    yline(0) xline(-1)
graph export "LPseparate_Gm4_CV1(95)ct_impulse_eng_short.png", replace width(3000)

* The normalized (norm) PNG block was removed; the Dbar columns stay in res.csv so it can be computed afterwards.
