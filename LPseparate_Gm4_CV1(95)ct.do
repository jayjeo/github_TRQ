...
forvalues h = -`Hpre'/`Hpost' {
    ...
    * Clean controls: new treatment (clean event) vs clean control (prevtr120==0)
    ...
    if (`Nall'==0 | `NT'==0 | `Nc'==0) {
        ...
        gen double shock = intensity*`ev'*(d==1)*(prevtr120==0)*(`tmin'==1)
...
export delimited using "LPseparate_Gm4_CV1(95)ct_res.csv", replace

* The CSV keeps every horizon (-500~273); only the graph is cut at h<=`Hcut'. The vegetable spike from h=156 on is a composition effect (only radish and napa cabbage remain), so a causal reading is not appropriate.
keep if h <= `Hcut'
...
twoway ///
    ...
    xlabel(-`Hpre'(50)`Hcut') xscale(range(-`Hpre' `Hcut')) ///
    xtitle("h (상대시점, 일)") ///
    ytitle("Log 소매가격 반응 (강도 1%p당)") ///
    legend(order(3 "처치그룹1" 4 "처치그룹2") pos(6) ring(0)) ///
    yline(0) xline(-1)
graph export "LPseparate_Gm4_CV1(95)ct.png", replace width(3000)
...
twoway ///
    ...
    yline(0) xline(-1)
graph export "LPseparate_Gm4_CV1(95)ct_eng.png", replace width(3000)
...
twoway ///
    ...
    legend(order(3 "Treated Group 1" 4 "Treated Group 2") pos(6) ring(0)) ///
    yline(0) xline(-1)
graph export "LPseparate_Gm4_CV1(95)ct_eng_short.png", replace width(3000)
