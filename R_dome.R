## ===== R: Daily wholesale price seasonality removal (panel, keep 품목코드) =====
dir_path <- "D:/JJ Dropbox/KCTDI_Research/할당관세 정책이 소비자 물가에 미치는 영향/GItPublish_3rd_submit"
in_path  <- file.path(dir_path, "q_all_daily_wholesale.csv")
out_path <- file.path(dir_path, "q_all_daily_wholesale_STL_result.csv")
hol_path <- file.path(dir_path, "kr_holidays_2021_2025.csv")
...
stopifnot(all(c("품목코드","dome","dome2","date","y") %in% names(df)))
df$date <- as.Date(df$date)
df$y    <- suppressWarnings(as.numeric(df$y))
df <- df[order(df$dome2, df$date), ]
...
remove_daily_season <- function(g){
  ...
  if (!any(idx) || sum(idx) < 120) {
    ...
    return(g[, c("품목코드","dome","dome2","date","y","y_sa_add","y_sa_mul")])
  }
  g2 <- g[idx, , drop=FALSE]; n <- nrow(g2)
  
  dow    <- factor(strftime(g2$date, "%u"), levels=c("1","2","3","4","5","6","7"))
  X_dow  <- model.matrix(~ dow - 1)
  doy    <- as.integer(strftime(g2$date, "%j"))
  K      <- min(24, max(8, floor(length(unique(doy))/6)))
  X_four <- fourier_doy(doy, K=K)
  ...
  if (is.null(fitM)) { g$y_sa_mul <- NA_real_ } else {
    ...
    seas_log <- as.numeric(X %*% b) - mean(as.numeric(X %*% b))
  ...
  if (is.null(fitA)) { g$y_sa_add <- NA_real_ } else {
    ...
    seas_lin <- as.numeric(X %*% bA) - mean(as.numeric(X %*% bA))
    g$y_sa_add <- NA_real_; g$y_sa_add[idx] <- g2$y - seas_lin
  }
  
  g[, c("품목코드","dome","dome2","date","y","y_sa_add","y_sa_mul")]
}

res <- do.call(rbind, lapply(split(df, df$dome2), remove_daily_season))
