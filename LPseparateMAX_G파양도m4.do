...
** 호출 전: d, TRQD, qcode, date, d_price 존재 가정
************************************************
...
forvalues h = -`Hpre'/`Hpost' {
    ...
    * 클린 컨트롤: 신규처리 vs (위에서 정의한) clean control
    ...
    if (`Nall'==0 | `NT'==0 | `Nc'==0) {
        ...
        quietly reg `dX' shock i.date BaseTax i.qcode#c.oil_price i.qcode#c.temp_avg i.qcode#c.humidity_avg i.qcode#c.precipitation_daily i.qcode#c.sunshine_hours i.qcode#c.L365.temp_avg i.qcode#c.L365.humidity_avg i.qcode#c.L365.precipitation_daily i.qcode#c.L365.sunshine_hours i.month, vce(cluster qcode)   
        post `post1' (`h') (`Nall') (`NT') (`Nc') ///
...
twoway ///
    ...
    xtitle("h (상대시점, 일)") ///
    ytitle("Log 도매가격 반응 (강도 1%p당)") ///
    legend(order(3 "처치그룹1" 4 "처치그룹2") pos(6) ring(0)) ///
    yline(0) xline(-1)
graph export LPseparateMAX파양도_Gm4.png, replace width(3000)
...
twoway ///
    ...
    ytitle("Log Wholesale Price Response (per 1%p Intensity)") ///
    legend(order(3 "Treated Group 1" 4 "Treated Group 2") pos(6) ring(0)) ///
    yline(0) xline(-1)
graph export LPseparateMAX파양도_Gm4_eng.png, replace width(3000)
